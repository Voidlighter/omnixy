    ...
    in {
      ...
      devShells.${system} = {
        default = pkgs.mkShell {
          ...
          shellHook = ''
            ...
            # Setup aliases
            alias omnixy-rebuild="sudo nixos-rebuild switch --flake .#veridia"
      ...
      apps.${system} = {
        ...
        installer = {
          type = "app";
          program = "${
              pkgs.writeShellScriptBin "omnixy-install" ''
                #!/usr/bin/env bash
                set -e
        ...
        build-iso = {
          type = "app";
          program = "${
              pkgs.writeShellScriptBin "omnixy-build-iso" ''
                #!/usr/bin/env bash
                set -e
